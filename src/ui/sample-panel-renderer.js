...
export function renderProjectSampleUsageHtml(usage = []) {
  ...
  return usage
    .map((slot) => {
      ...
      return `<article class="sample-slot-card state-${escapeHtml(slot.availability)}" data-sample-slot="${escapeHtml(slot.id)}" data-sample-ref="${escapeHtml(slot.sampleRef)}" data-filename="${escapeHtml(slot.filename)}" style="--sample-fill:${progress}%"><div class="sample-slot-fill"></div><strong>${escapeHtml(slot.filename)}</strong><small>${escapeHtml(slot.moduleTitle)} · ${escapeHtml(slot.sampleRef)}</small><span class="pill">${escapeHtml(slot.availability)}</span><span class="microcopy">${escapeHtml(slot.sampleLengthMs || '?')}ms · ${escapeHtml(slot.type || 'unknown type')}</span><div class="button-row"><button type="button" data-sample-action="query-peer">QUERY PEERS</button><button type="button" data-sample-action="pick-upload">UPLOAD / REPLACE</button></div></article>`;
    })
    .join('');
}
